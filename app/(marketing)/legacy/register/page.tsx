...
export default function LegacyRegisterPage() {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    setError("")
    try {
      const res = await fetch("/api/legacy/auth/register", {
        method: "POST",
  ...
  return (
    <section className="container flex min-h-[70vh] items-center justify-center py-10">
      <div className="w-full max-w-md rounded-lg border p-6">
        <h1 className="mb-6 text-2xl font-semibold">Legacy 注册</h1>
        ...
        <p className="mt-4 text-sm text-muted-foreground">
          已有账号？ <Link href="/legacy/login" className="underline">去登录</Link>
        </p>
      </div>
    </section>
  )
